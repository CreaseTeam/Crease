...
namespace Crease.Flying.Environment.Wind.Visuals
{
    ...
    public class PaperRibbonWindVfx : MonoBehaviour
    {
        ...
        private static readonly int SpawnMinId = Shader.PropertyToID("SpawnMin");
        private static readonly int SpawnMaxId = Shader.PropertyToID("SpawnMax");
        ...
        private bool _hasSpawnMin;
        private bool _hasSpawnMax;
        ...
        private void OnEnable()
        {
            ...
            if (Application.isPlaying) ReportSetupProblems();
        }

        // Silent nothing-on-screen is the worst failure mode here, so say why once.
        private void ReportSetupProblems()
        {
            if (_vfx.visualEffectAsset == null)
            {
                Debug.LogError("PaperRibbonWindVfx on '" + name + "' has no Visual Effect asset assigned.", this);
                return;
            }

            if (!_hasSpawnRate && !_hasSpawnMin && !_hasFlowVelocity)
            {
                Debug.LogError(
                    "PaperRibbonWindVfx on '" + name + "' found none of its exposed properties on '" +
                    _vfx.visualEffectAsset.name + "'. The graph blackboard names must match, " +
                    "so regenerate it with Tools/Crease/Paper Ribbons.", this);
                return;
            }
            ...
            {
                Debug.LogWarning(
                    "PaperRibbonWindVfx on '" + name + "' has no follow target and no camera is " +
                    "tagged MainCamera, so ribbons will spawn around the object instead of the player.", this);
            }
        }
        ...
        private void CacheProperties()
        {
            ...
            _hasSpawnMin = _vfx.HasVector3(SpawnMinId);
            _hasSpawnMax = _vfx.HasVector3(SpawnMaxId);
        ...
        private void Push(Vector3 anchor, Vector3 travelDir)
        {
            ...
            // The graph spawns at a random point between two corners, so the box is
            // sent as world space corners rather than a centre and a size.
            Vector3 centre = anchor + travelDir * (_spawnBoxSize.z * _spawnAhead);
            Vector3 half = _spawnBoxSize * 0.5f;
            if (_hasSpawnMin) _vfx.SetVector3(SpawnMinId, centre - half);
            if (_hasSpawnMax) _vfx.SetVector3(SpawnMaxId, centre + half);
