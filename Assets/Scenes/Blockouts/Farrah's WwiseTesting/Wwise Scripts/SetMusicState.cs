...
public class SetMusicState : MonoBehaviour
{
    ...
    public static List<AK.Wwise.State> RegionStates = new List<AK.Wwise.State>();
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            RegionStates.Insert(0, OnTriggerEnterState);
            Debug.Log("SetMusicState - RegionStates list size after entering region is now " + RegionStates.Count);
            Debug.Log($"SetMusicState - [{string.Join(", ", RegionStates.Select(obj => obj != null ? obj.Name : "NULL"))}]");
            RegionStates[0].SetValue();
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            RegionStates.Remove(OnTriggerEnterState);
            Debug.Log("SetMusicState - RegionStates list size after region removal is now " + RegionStates.Count);
            Debug.Log($"SetMusicState - [{string.Join(", ", RegionStates.Select(obj => obj != null ? obj.Name : "NULL"))}]");
